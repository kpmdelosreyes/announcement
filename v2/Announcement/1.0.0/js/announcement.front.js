	...
	init : function(){
                ...
                PLUGIN.post(pNode, mData, 'custom', 'json', function(serverResponse){
                    ...
                    if (PG_Announcement.rowStamp != serverResponse.rows)
                    {
                        ...
                        $(".showHide").html("").css({"padding":"0", "border":"0", "height":""});
                        var string = '';             
                        ...
                        $.each(serverResponse.aData, function(key, val){
                            ...
                            string += '<h3>';
                            string += '<a href="#none" class="pg_announce_title" rel="'+val.id+'" onclick="PG_Announcement.showThis(this);"><span class="pg_announce_h">'+val.title+'</span></a>';
                            string += '</h3>';
                            string += '<div class="content_id"><p class="showHide" style="border:1px solid #ddd;background:#fff;max-height:150px; overflow : auto; padding:5px;width : 220px;margin-bottom:2px"> '+val.content+' <span class="timeago" title="'+val.date+'">'+date+'</span></p> </div></li>';
        ...
        showThis : function (obj){
            ...
            if ($(obj).parents("li").find(".content_id p").is(":visible") === true){
                ...
                PLUGIN.post(pNode, mData, 'custom', 'json', function(serverResponse){
                    ...
                    $(obj).parent().parent().find('.content_id p').html(serverResponse.pad_content).css({"border":"1px solid #ddd","max-height" : "150px", "overflow" : "auto", "padding":"5px", "width" : "220px", "display":"inline-block", "background":"#fff", "margin":"0 0 2px 0"});
